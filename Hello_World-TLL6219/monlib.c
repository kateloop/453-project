...
static void		(*_mondelay)(unsigned long);
	...
	void (*lock)(void), void (*unlock)(void))
{
	...
	if (mon) {
		...
		_moncom(GETMONFUNC_SENDENETPKT,&_sendenet,0,0);
		_moncom(GETMONFUNC_RECVENETPKT,&_recvenet,0,0);
		_moncom(GETMONFUNC_GETSYM,&_getsym,0,0);
		_moncom(GETMONFUNC_PRINTPKT,&_printpkt,0,0);
		_moncom(GETMONFUNC_FLASHWRITE,&_flashwrite,0,0);
		_moncom(GETMONFUNC_FLASHERASE,&_flasherase,0,0);
		_moncom(GETMONFUNC_FLASHINFO,&_flashinfo,0,0);
		_moncom(GETMONFUNC_ASSIGNHDLR,&_assign_handler,0,0);
		_moncom(GETMONFUNC_WATCHDOG,&_watchdog,0,0);
		_moncom(GETMONFUNC_PRINTMEM,&_printmem,0,0);
	}
}
